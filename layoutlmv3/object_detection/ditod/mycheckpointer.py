...
def modify_ckpt_state(model, state_dict, logger=None):
    ...
    def resize_position_embeddings(max_position_embeddings, old_vocab_size, _k='backbone.bottom_up.backbone.embeddings.position_embeddings.weight',
        ...
        new_position_embedding = state_dict[_k].data.new_tensor(
            torch.ones(
                size=(max_position_embeddings, state_dict[_k].shape[1])
            ),
            dtype=torch.float
        )
