...
class LayoutLMv3SelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):  # 要求隐层SIZE必须是注意力头数量的整数倍
        ...
        self.attention_head_size = int(config.hidden_size / config.num_attention_heads)  # 64 # 每个注意力头只负责一部分数据
...
class LayoutLMv3Encoder(nn.Module):
    ...
    def __init__(self, config, detection=False, out_features=None):
        ...
        self.layer = nn.ModuleList([LayoutLMv3Layer(config) for _ in range(config.num_hidden_layers)])  # 12
...
class LayoutLMv3Model(LayoutLMv3PreTrainedModel):  # 作为BACKBONE模型
    """
    """
    ...
    def __init__(self, config, detection=False, out_features=None, image_only=False):
        ...
        self.config = config  # LayoutLMv3Config
        ...
        self.encoder = LayoutLMv3Encoder(config, detection=detection, out_features=out_features) # 编码器
    ...
    def _init_visual_bbox(self, img_size=(14, 14), max_len=1000):
        ...
        visual_bbox_x = torch.div(torch.arange(0, max_len * (img_size[1] + 1), max_len), img_size[1], rounding_mode='trunc')
        visual_bbox_y = torch.div(torch.arange(0, max_len * (img_size[0] + 1), max_len), img_size[0], rounding_mode='trunc')
